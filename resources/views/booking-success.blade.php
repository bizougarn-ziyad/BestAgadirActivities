            ...
            <div class="bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden">
                ...
                <div class="px-8 py-12">
                                    ...
                                    <div class="bg-white p-6 rounded-xl border border-gray-200">
                                        <div class="flex items-center gap-3 mb-4">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-lg font-semibold text-gray-800">{{ $order->activity->name }}</p>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div class="bg-white p-6 rounded-xl border border-gray-200">
                                    <div class="flex items-center gap-3 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-lg font-semibold text-gray-800">{{ $order->booking_date ? $order->booking_date->format('F j, Y') : 'N/A' }}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="bg-white p-6 rounded-xl border border-gray-200">
                                    <div class="flex items-center gap-3 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-lg font-semibold text-gray-800">{{ $order->participants }} {{ $order->participants == 1 ? 'person' : 'people' }}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="bg-white p-6 rounded-xl border border-gray-200">
                                    <div class="flex items-center gap-3 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-lg font-semibold text-gray-800">${{ number_format($order->price_per_person, 2) }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="bg-orange-50 p-6 rounded-xl border-2 border-orange-200">
                                    <div class="text-center">
                                        ...
                                        <p class="text-3xl font-bold text-orange-800">${{ number_format($order->total_price, 2) }}</p>
                                    </div>
                                </div>
                                ...
                                <div class="bg-blue-50 p-6 rounded-xl border-2 border-blue-200">
                                    <div class="text-center">
                                        ...
                                        <p class="text-2xl font-mono font-bold text-blue-800">{{ strtoupper(substr($order->session_id, -8)) }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-green-50 rounded-2xl p-8 mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <p class="text-sm font-mono text-gray-800 bg-white px-3 py-2 rounded-lg border break-all overflow-hidden">{{ $session->id }}</p>
                                </div>
                                <div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-center">
                        ...
                        <a href="{{ url('/') }}" class="bg-orange-500 hover:bg-orange-600 text-white px-8 py-4 rounded-2xl font-semibold text-center transition duration-300 shadow-lg hover:shadow-xl cursor-pointer">
                            <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path>
                            </svg>
                            Return to Home
                        </a>
                        <a href="{{ route('activities') }}" class="bg-white border-2 border-orange-500 text-orange-500 hover:bg-orange-50 px-8 py-4 rounded-2xl font-semibold text-center transition duration-300 cursor-pointer">
                            <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        function savePaymentDetails() {
            ...
            const bookingDetails = {
                @if(isset($order) && $order)
                activity: "{{ $order->activity ? $order->activity->name : 'N/A' }}",
                bookingDate: "{{ $order->booking_date ? $order->booking_date->format('F j, Y') : 'N/A' }}",
                participants: "{{ $order->participants }}",
                pricePerPerson: "${{ number_format($order->price_per_person, 2) }}",
                totalPaid: "${{ number_format($order->total_price, 2) }}",
                bookingReference: "{{ strtoupper(substr($order->session_id, -8)) }}",
                @endif
                @if(isset($session))
                sessionId: "{{ $session->id }}",
                paymentStatus: "{{ ucfirst($session->payment_status) }}",
                @endif
            ...
            a.href = url;
            a.download = 'Payment_Confirmation_{{ isset($order) && $order ? strtoupper(substr($order->session_id, -8)) : "Details" }}.txt';

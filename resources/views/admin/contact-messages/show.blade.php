    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600">Message from {{ $contactMessage->first_name }} {{ $contactMessage->last_name }}</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6">
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            <h2 class="text-xl font-semibold text-gray-800 mb-2">
                                {{ ucfirst(str_replace('_', ' ', $contactMessage->subject)) }}
                            </h2>
                            <div class="flex items-center text-sm text-gray-500 space-x-4">
                                <span>{{ $contactMessage->created_at->format('M d, Y \a\t H:i') }}</span>
                                <span>•</span>
                                <span>{{ $contactMessage->created_at->diffForHumans() }}</span>
                            </div>
                        </div>
                        <div>
                ...
                <div class="border-t border-gray-200 pt-6">
                    ...
                    <div class="flex flex-wrap gap-3">
                        <!-- Email Reply -->
                        <a href="mailto:{{ $contactMessage->email }}?subject=Re: {{ urlencode($contactMessage->subject) }}&body={{ urlencode('Hello ' . $contactMessage->first_name . ',\n\nThank you for contacting Best Agadir Activities. \n\nRegarding your inquiry about: ' . $contactMessage->subject . '\n\n') }}"
                        ...
                        @if($contactMessage->phone)
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $contactMessage->phone) }}?text={{ urlencode('Hello ' . $contactMessage->first_name . ', thank you for contacting Best Agadir Activities regarding: ' . $contactMessage->subject) }}"
                               target="_blank"
            ...
            <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <p class="text-gray-800">{{ $contactMessage->first_name }} {{ $contactMessage->last_name }}</p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-gray-500">Email</label>
                        <p class="text-gray-800">{{ $contactMessage->email }}</p>
                    </div>
                    @if($contactMessage->phone)
                        <div>
                            <label class="text-sm font-medium text-gray-500">Phone</label>
                            <p class="text-gray-800">{{ $contactMessage->phone }}</p>
                        </div>
                    @endif
                    <div>
                        <label class="text-sm font-medium text-gray-500">User Account</label>
                        <p class="text-gray-800">{{ $contactMessage->user->name ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-gray-500">Messages Today</label>
                        <p class="text-gray-800">{{ \App\Models\ContactMessage::getTodayMessageCount($contactMessage->user_id) }}/3</p>
                    </div>
                </div>
            </div>

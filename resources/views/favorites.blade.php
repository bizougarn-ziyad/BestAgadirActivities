        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if($favoriteActivities->count() > 0)
                ...
                <div class="flex justify-between items-center mb-12">
                    <h2 class="text-2xl font-bold text-gray-800">{{ $favoriteActivities->total() }} Favorite{{ $favoriteActivities->total() !== 1 ? 's' : '' }}</h2>
                    <a href="{{ route('activities') }}" class="bg-gradient-to-r from-orange-500 to-orange-600 text-white font-semibold py-2 px-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 transform hover:scale-105">
                        ...
                        <div class="group relative bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100 flex flex-col h-full">
                            ...
                            <div class="relative h-64 overflow-hidden flex-shrink-0">
                                @if($activity->hasImageData())
                                    <img src="{{ $activity->image_data_url }}" alt="{{ $activity->name }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300">
                                @else
                                ...
                                <button class="absolute top-4 right-4 w-10 h-10 bg-white/90 hover:bg-white rounded-full flex items-center justify-center shadow-lg transition-all duration-300 favorite-btn" data-activity-id="{{ $activity->id }}">
                                ...
                                <div class="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full">
                                    <span class="font-bold text-orange-600">€{{ number_format($activity->price, 0) }}</span>
                                </div>
                            </div>

                            <!-- Activity Content -->
                            <div class="p-6 flex-grow flex flex-col">
                                <h3 class="text-xl font-bold text-gray-800 mb-4 group-hover:text-orange-600 transition-colors">{{ $activity->name }}</h3>
                ...
                <!-- Pagination -->
                @if($favoriteActivities->hasPages())
                    <div class="flex justify-center">
                        <div class="bg-white rounded-xl shadow-lg p-4">
                            {{ $favoriteActivities->links() }}
                        </div>
                    </div>
                @endif
            ...
            @else
                ...
                <div class="text-center py-20">
                    ...
                    <a href="{{ route('activities') }}" class="inline-flex items-center bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-semibold py-3 px-6 rounded-xl transition-all duration-300 transform hover:scale-105 shadow-md hover:shadow-lg">
